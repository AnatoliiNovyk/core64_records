...
- runtime config validation runs centralized DATABASE_URL policy helper (`scripts/check-database-url-policy.mjs --strict`) and fails fast on invalid format or missing pooler `sslmode=require` (before Cloud Run rollout)
...
- pre-smoke deploy gate runs strict DB route compatibility verdict (`scripts/print-cloud-run-db-route-verdict.mjs --strict`) to fail fast on connector/egress mismatches and missing `sslmode=require` for pooler endpoints
...
- pre-smoke rollback gate runs strict DB route compatibility verdict (`scripts/print-cloud-run-db-route-verdict.mjs --strict`) before retries (including pooler `sslmode=require` validation)
