...
- `/api/health/db` now also includes a safe connectivity probe (`probe.dns` + `probe.tcp`) for timeout/connection incidents to distinguish DNS failure vs TCP reachability failure from runtime
- deploy/rollback smoke loops now fail fast (skip retries) for non-transient DB errors (`healthDb.kind=auth|config` or `dbCode=28P01`) and print credential-focused remediation guidance
